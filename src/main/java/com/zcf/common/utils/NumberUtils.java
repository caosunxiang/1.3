...
public class NumberUtils {
	 ...
	 * @return
	 */
	public static double calPercent(int num1, int num2) {
		double result = 0.00;
		if (num2 == 0) { // 除数不能为0
			return result;
		}
		...
		return big1.divide(big2, 4, BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(100)).doubleValue();
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(
				NumberUtils.compare(NumberUtils.sub(new BigDecimal(4900.23), new BigDecimal(4000)), BigDecimal.ZERO));
		/*
		 * long lang=1; String str1=Long.toBinaryString(lang);//将给定的数转为二进制字串
		 * 
		 * System.out.println(str1);
		 * 
		 * String str2=Long.toBinaryString(~lang);//将给定的数取反转为二进制字串 //将取反后的二进制串取最后的
		 * 长度与第一个串长度相等 的子串，这是我们需要的二进制串 String
		 * str=str2.substring(str2.length()-str1.length()); //将它转换为十进制整数 int
		 * i=Integer.parseInt(str, 2); System.out.println(i);
		 * 
		 * int temp= 1; temp=~temp; temp=temp&0xFFFFFFFF; System.out.println(temp);
		 * System.out.println(0&0XFF); String aa = Long.toBinaryString(~1);
		 * System.out.println(aa);
		 * System.out.println(NumberUtil.binaryString2hexString(aa)); String ad = "";
		 * String sa = "0000000000000111"; for(int j = 0;j<sa.length();j++){ String asdf
		 * = sa.substring(j,j+1); System.out.println(asdf);;
		 * //System.out.println(RfM1CardAction.binaryString2hexString(Long.
		 * toBinaryString(~Integer.valueOf(asdf)))); ad = ad
